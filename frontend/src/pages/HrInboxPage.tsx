...
import HrActionModal from "../components/HrActionModal";

import { getHrPendingLeaves } from "../services/approval.service";
...
const HrInboxPage = () => {
  const [pendingLeaves, setPendingLeaves] =
    useState<HrPendingLeave[]>([]);

  const [selectedAction, setSelectedAction] =
    useState<SelectedHrAction | null>(null);
        ...
        if (
          ...
          setErrorMessage(
            error.response?.data.message ??
              "Unable to load HR pending requests."
          );
        } else {
          setErrorMessage(
            "An unexpected error occurred while loading the HR Inbox."
          );
        }
      } finally {
        setIsLoading(false);
  ...
  if (isLoading) {
    return (
      <section className="manager-inbox-state">
        ...
        <h1>Loading HR Inbox</h1>

        <p>
          Please wait while Manager leave requests
          are loaded.
        </p>
      </section>
    );
  ...
  return (
    <>
      <section className="manager-inbox-page">
        <header className="manager-inbox-header">
          <div>
            ...
            <p>
              Review Manager leave requests assigned
              to you.
            </p>
          </div>
        ...
        <article className="manager-inbox-summary-card">
          ...
          <div>
            ...
            <p>
              Manager requests waiting for your
              review.
            </p>
          </div>
        </article>
        ...
        {pendingLeaves.length === 0 ? (
          <section className="manager-inbox-empty">
            ...
            <p>
              New Manager leave requests assigned to
              HR will appear here.
            </p>
          </section>
        ) : (
      ...
      {selectedAction !== null && (
        ...
        <HrActionModal
        leaveRequest={selectedAction.leaveRequest}
